...
int sndResult;	// リザルトBGM
int sndSe[MAX_SE];	// SE[MAX_SE]
...
void InitSound()
{
	...
	// 曲データの読み込み（ストリーム）
	sndBgm = DSoundLoadFile("sound/re-su.wav", -1, DSLF_FLG_STREAM);
	sndTitle = DSoundLoadFile("sound/title.wav", -1, DSLF_FLG_STREAM);
	sndResult = DSoundLoadFile("sound/result.wav", -1, DSLF_FLG_STREAM);

	// SEデータの読み込み（スタティック）
	sndSe[ATTACK] = DSoundLoadFile("sound/SE/attack.wav", -1, DSLF_FLG_STATIC);
	sndSe[CANCEL] = DSoundLoadFile("sound/SE/cancel.wav", -1, DSLF_FLG_STATIC);
	sndSe[CRAFT] = DSoundLoadFile("sound/SE/craft.wav", -1, DSLF_FLG_STATIC);
	sndSe[DIE] = DSoundLoadFile("sound/SE/die.wav", -1, DSLF_FLG_STATIC);
	sndSe[DECISION] = DSoundLoadFile("sound/SE/ok.wav", -1, DSLF_FLG_STATIC);
	sndSe[ROCK] = DSoundLoadFile("sound/SE/rock.wav", -1, DSLF_FLG_STATIC);
	sndSe[SELECT] = DSoundLoadFile("sound/SE/select.wav", -1, DSLF_FLG_STATIC);
	sndSe[WOOD] = DSoundLoadFile("sound/SE/wood.wav", -1, DSLF_FLG_STATIC);
	sndSe[PICK] = DSoundLoadFile("sound/SE/pick.wav", -1, DSLF_FLG_STATIC);
...
void StartSE(int type)
{
	...
	case ATTACK:
		// SE[0]を再生(攻撃)
		DSoundStop(sndSe[ATTACK]);
		DSoundPlay(sndSe[ATTACK], FALSE);
		break;
	case CANCEL:
		// SE[1]を再生(キャンセル)
		DSoundStop(sndSe[CANCEL]);
		DSoundPlay(sndSe[CANCEL], FALSE);
		break;
	case CRAFT:
		// SE[2]を再生(クラフト)
		DSoundStop(sndSe[CRAFT]);
		DSoundPlay(sndSe[CRAFT], FALSE);
		break;
	case DIE:
		// SE[3]を再生(死亡)
		DSoundStop(sndSe[DIE]);
		DSoundPlay(sndSe[DIE], FALSE);
		break;
	case DECISION:
		// SE[4]を再生(決定)
		DSoundStop(sndSe[DECISION]);
		DSoundPlay(sndSe[DECISION], FALSE);
		break;
	case ROCK:
		// SE[5]を再生(岩)
		DSoundStop(sndSe[ROCK]);
		DSoundPlay(sndSe[ROCK], FALSE);
		break;
	case SELECT:
		// SE[6]を再生(セレクト)
		DSoundStop(sndSe[SELECT]);
		DSoundPlay(sndSe[SELECT], FALSE);
		break;
	case WOOD:
		// SE[7]を再生(木)
		DSoundStop(sndSe[WOOD]);
		DSoundPlay(sndSe[WOOD], FALSE);
		break;
	case PICK:
		// SE[8]を再生(拾う)
		DSoundStop(sndSe[PICK]);
		DSoundPlay(sndSe[PICK], FALSE);
		break;
	default:
		break;
	}
