...
//	アニメーション
//------------------------------------------------
void FbxMeshKM::Animate(float sec)
{
	float DeltaTime = sec;
	//	モーション時間の更新
	Frame += DeltaTime * 60;
...
void FbxMeshKM::Render() {
	//IDirect3DDevice9* device = DX9System::Device;
	IDirect3DDevice9* device = gDevice;
	...
	for (int m = 0; m < NumMesh; m++) {
		...
		device->DrawIndexedPrimitiveUP(
			D3DPT_TRIANGLELIST,     // PrimitiveType - プリミティブを指定します
			0,                      // MinIndex - 使用するインデックスの最小値を指定します
			NumVertices,            // NumVerticex - このレンタリングで使用される頂点の数を指定します
			MaterialFaces[m],       // PrimitiveCount - 描画するプリミティブの数を指定します
			&Indices[start],        // pIndexData - インデックスデータの数値配列へのポインタを指定します
			D3DFMT_INDEX32,         // IndexDataFormat - D3DFORMAT 列挙型メンバを指定します
			Vertices,               // pVertexStreamZeroData - 頂点データの配列へのポインタを指定します
			sizeof(PolygonVertex)); // VertexStreamZeroStride - 頂点でータ一つ分のサイズをバイト単位で指定します
...
void FbxMeshKM::Create(const char* filename)
{
	//ファイル名を取り除く
...
void FbxMeshKM::Load(const char* filename)
{
...
//	座標読み込み
//------------------------------------------------
void FbxMeshKM::LoadPosition(FbxMesh* mesh) {
...
//	法線読み込み
//------------------------------------------------
void FbxMeshKM::LoadNormal(FbxMesh* mesh) {
...
//	ＵＶ読み込み
//------------------------------------------------
void FbxMeshKM::LoadUV(FbxMesh* mesh) {
	FbxStringList names;
...
//	頂点カラー読み込み
//------------------------------------------------
void FbxMeshKM::LoadVertexColor(FbxMesh* mesh) {
...
//	材質読み込み
//****************************************************************
void FbxMeshKM::LoadMaterial(int index, FbxSurfaceMaterial * material)
{
	//IDirect3DDevice9* device = DX9System::Device;
	IDirect3DDevice9* device = gDevice;
	...
	//NTextures[index] = LoadTexture(filename);
...
//	ボーン検索
//****************************************************************
int FbxMeshKM::FindBone(const char* name)
{
	int bone = -1; // 見つからない
...
void FbxMeshKM::LoadBone(FbxMesh* mesh)
{
	//	メッシュ頂点数
	int num = mesh->GetPolygonVertexCount();
...
//	ボーンのないメッシュのアニメーション
void FbxMeshKM::LoadMeshAnim(FbxMesh* mesh)
{
...
//	キーフレーム読み込み
void FbxMeshKM::LoadKeyFrames(std::string name, int bone, FbxNode* bone_node) {
	//	メモリ確保
...
void FbxMeshKM::AddMotion(std::string name, const char * filename)
{
	FbxManager* manager = FbxManager::Create();
	FbxScene* scene = FbxScene::Create(manager, "");
	//	ファイルからシーンに読み込み
...
//	頂点最適化
//****************************************************************
void FbxMeshKM::OptimizeVertices()
{
	int currentNum = 0;
	for (int v = 0; v < NumVertices; v++) {
		int sameIndex = -1;
...
//	レイ判定
int	FbxMeshKM::Raycast(const D3DXVECTOR3& pos, const D3DXVECTOR3& vec, float Dist, D3DXVECTOR3& out, D3DXVECTOR3& normal)
{
	int		ret = -1;
...
int FbxMeshKM::Raycast2(const D3DXVECTOR3& pos, const D3DXVECTOR3& vec, float Dist, D3DXVECTOR3& out, D3DXVECTOR3& normal) {
	//	逆行列
	D3DXMATRIX Inv;
	D3DXMatrixInverse(&Inv, NULL, &transform);

	D3DXVECTOR3 P;
	D3DXVec3TransformCoord(&P, &pos, &Inv);

	D3DXVECTOR3 V;
	D3DXVec3TransformNormal(&V, &vec, &Inv);
	D3DXVECTOR3 O;
	D3DXVECTOR3 N;
	int ret = Raycast(P, V, Dist, O, N);
	if (ret >= 0) {
		D3DXVec3TransformCoord(&out, &O, &transform);
		D3DXVec3TransformNormal(&normal, &N, &transform);
	}
	return ret;
}
